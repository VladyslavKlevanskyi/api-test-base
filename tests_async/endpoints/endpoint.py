...
class Endpoint:
    """
    Base class for asynchronous API endpoint tests.
    ...
    def __init__(self, async_client: AsyncClient) -> None:
        """
        Initialize the endpoint helper with an async client.

        Args:
            async_client (AsyncClient): The HTTPX asynchronous client.
        """

        self.client: AsyncClient = async_client
        self.url_root: str = "/"
        self.url_apartments: str = "/apartments"
        self.url_users: str = "/users"
    ...
    async def _get_authorization_headers(self) -> dict[str, str]:
        ...
        """
        headers = {
            "Authorization": "Bearer " + await get_valid_token(
    ...
    async def _get_admin_authorization_headers(self) -> dict[str, str]:
        """
    ...
    def check_that_status_is(
        ...
        if error_message is not None:
            assert self.response.status_code == code, (
                error_message, self.response.status_code
            )
        assert self.response.status_code == code, (
    ...
    def check_field_validation_error_response(
        ...
        """
        Assert that field-level validation error matches expected field and
        message.
