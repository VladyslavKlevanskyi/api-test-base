...
async def test_retrieve_apartment_by_unit_id(
    ...
    await retrieve_apartment_endpoint.retrieve_apartment_by_unit_id(
        unit_id=apartment_data["unit_id"]
    )
    # Assertions
...
async def test_retrieve_apartment_by_unit_id_with_invalid_headers(
        create_apartment_endpoint: CreateApartment,
        retrieve_apartment_endpoint: RetrieveApartment,
...
@pytest.mark.negative
@pytest.mark.asyncio
async def test_retrieve_apartment_by_incorrect_unit_id(
        retrieve_apartment_endpoint: RetrieveApartment
) -> None:
    unit_id = 92354316  # Non existent apartment unit_id
    await retrieve_apartment_endpoint.retrieve_apartment_by_unit_id(
        unit_id=unit_id
    )
    # Assertions
    retrieve_apartment_endpoint.check_that_status_is(404)
    retrieve_apartment_endpoint.check_apartment_not_found_message(
