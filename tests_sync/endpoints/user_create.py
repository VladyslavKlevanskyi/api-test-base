...
class CreateUser(Endpoint):
    """
    Endpoint test helper for creating a new user via /users/register.

    Inherits from the base Endpoint class and provides functionality
    ...
    def register_new_user(
        ...
        """
        Register a new user by sending a POST request to /users/register.

        Args:
            payload (dict[str, Any]): Dictionary containing user registration
            data.
            headers (dict | None): Optional headers. If not provided,
